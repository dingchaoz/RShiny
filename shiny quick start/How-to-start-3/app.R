...
server <- function(input, output) {
  ...
  output$norm <- renderPlot({
    hist(rv$norm, breaks = 30, col = "grey", border = "white",
      main = "Welcome to 500 random draws from a standard normal distribution - Made by Dingchao Zhang")
  })
  output$unif <- renderPlot({
    hist(rv$unif, breaks = 30, col = "grey", border = "white",
      main = "Welcome to  500 random draws from a standard uniform distribution - Made by Dingchao Zhang")
  })
  output$chisq <- renderPlot({
    hist(rv$chisq, breaks = 30, col = "grey", border = "white",
       main = "Welcome to 500 random draws from a Chi Square distribution with two degree of freedom - Made by Dingchao Zhang")
  })
}
