...
ui <-  shinyUI(fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      plotOutput("distPlot",click = 'firstPlot'),
      plotOutput("distPlot2",click = 'secondPlot')
    )
  )
    ))
...
server <- shinyServer(function(input, output,session) {
  ...
  data1 <- reactive({input$firstPlot})
  data2 <- reactive({input$secondPlot})
  
  observe({if(is.null(input$firstPlot)) {
      shinyjs::hide(id = "distPlot")
      shinyjs::show(id = "distPlot2")
      
    } else if(is.null(input$secondtPlot))  {
